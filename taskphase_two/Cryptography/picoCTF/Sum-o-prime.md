# Challenge Sum-O-Primes
In this challenge, we get an encrypting code, and text file with three large hexadecimal numbers. 

It has three variables, x, which is the sum of p,q which are large prime numbers, c is the ciphertext and n is the RSA modulus, which is the most important part of RSA encryption. Larger values of n make it harder to find the prime factors.

The inverse code for the given python encryption code is: 

```python
import math


x =   # sum of p and q (in decimal)
n =   # modulus (in decimal)
c =   # ciphertext (in decimal)
e = 65537


discriminant = x * x - 4 * n
sqrt_discriminant = int(math.isqrt(discriminant))

p = (x + sqrt_discriminant) // 2
q = (x - sqrt_discriminant) // 2


m = (p - 1) * (q - 1) // math.gcd(p - 1, q - 1)

d = pow(e, -1, m)


FLAG_int = pow(c, d, n)


FLAG_hex = hex(FLAG_int)[2:]  
FLAG_text = bytes.fromhex(FLAG_hex).decode()

print("FLAG:", FLAG_text)

```

The basic math of this problem is the quadratic equation. We find both the factors using the quadratic equation and the relationships between the variables. 

flag=picoCTF{pl33z_n0_g1v3_c0ngru3nc3_0f_5qu4r35_92fe3557}

